@php
    ...
    $appliedCode = isset($latestBooking) && $latestBooking->referal_code ? $latestBooking->referal_code : session('booking_wizard.offer.code');
    $appliedDiscount = (float) (isset($latestBooking) && $latestBooking->discount_amount > 0 ? $latestBooking->discount_amount : session('booking_wizard.offer.discount_amount', 0));
...
<div class="booking-side-card booking-discount-offer-card mb-3"
    ...
    <div class="discount-type-group mb-3">
        ...
        <div class="discount-radios d-flex flex-column" style="gap: 10px;">
            <label class="discount-radio-card {{ !$hasActiveCode ? 'active' : '' }} p-3 border rounded-lg d-flex align-items-start m-0" for="offer-type-wallet" style="cursor: pointer;">
                <input type="radio" name="offer_type" value="wallet" id="offer-type-wallet" class="discount-radio-input mt-1" {{ !$hasActiveCode ? 'checked' : '' }} style="accent-color: #2563eb; width: 16px; height: 16px; flex-shrink: 0; margin-right: 14px;">
            ...
            <label class="discount-radio-card {{ $hasActiveCode ? 'active' : '' }} p-3 border rounded-lg d-flex align-items-start m-0" for="offer-type-promo" style="cursor: pointer;">
                <input type="radio" name="offer_type" value="promo" id="offer-type-promo" class="discount-radio-input mt-1" {{ $hasActiveCode ? 'checked' : '' }} style="accent-color: #2563eb; width: 16px; height: 16px; flex-shrink: 0; margin-right: 14px;">
                <div class="discount-card-body d-flex align-items-start">
                    <span class="discount-card-icon text-primary mr-2" style="font-size: 15px; margin-top: 1px; flex-shrink: 0; margin-left: 4px;">
    ...
    <div id="wallet-offer-panel" class="discount-panel-section" style="{{ !$hasActiveCode ? 'display: block;' : 'display: none;' }}">
        <!-- Wallet Summary Box -->
    ...
    <!-- Option 2: Referral Code Panel -->
    <div id="promo-offer-panel" class="discount-panel-section" style="{{ $hasActiveCode ? 'display: block;' : 'display: none;' }}">
        <h3 class="mb-2 font-weight-bold text-dark" style="font-size: 13px;">Have a Referral Code?</h3>

        @if ($hasCompletedReferralBooking && !$hasActiveCode)
            <div class="alert alert-danger p-2.5 mb-0 mt-2" style="font-size: 12px; border-radius: 8px; line-height: 1.45; background-color: #fef2f2; border-color: #fecaca; color: #991b1b;">
            ...
            <div id="promo-input-wrapper" style="{{ $hasActiveCode ? 'display: none;' : 'display: block;' }}">
                <div class="booking-promo-row">
                    <div class="booking-input-icon">
                        <i class="fas fa-tag" aria-hidden="true"></i>
                        <input type="text" id="promo-code-input" class="form-control" placeholder="Enter referral code">
                    </div>
